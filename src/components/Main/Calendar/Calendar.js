...
export default function Calendar(props) {
  ...
  function handleSwitchDate(e) {
    ...
    if (e.target.id === "month") {
      props.switchDate(
        document
          .querySelector(".calendar__year-active")
    ...
    if (e.target.id === "year") {
      props.switchDate(
        e.target.textContent,
        document.querySelector(".calendar__month-active").textContent
      );
    }
    setUnvisible();
  }
  ...
  return (
    <section className="calendar">
      <nav className="calendar__nav">
        <h2
          className="calendar__month-active"
          onMouseEnter={visibleCheker}
        ...
        <h2
          className="calendar__year-active"
          onMouseEnter={visibleCheker}
          onMouseLeave={setUnvisible}
        >
